...
def seriesAttributes(node): #===
	...
	# attributes['fitThumbSections'] = bool(node.get('fitThumbSection').capitalize())
	...
	# attributes['defaultBorder'] = [float(x) for x in node.get('defaultBorder').split(' ')]
	# attributes['defaultFill'] = [float(x) for x in node.get('defaultFill')]
	# attributes['defaultMode'] = int(node.get('defaultMode'))
	# attributes['defaultName'] = str(node.get('defaultName'))
	# attributes['defaultComment'] = str(node.get('defaultName'))
	...
	# attributes['ctrlIncrement'] = [float(x) for x in node.get('mvmtIncrement').split(' ')]
	# attributes['shiftIncrement'] = [float(x) for x in node.get('shiftIncrement').split(' ')]
	return attributes